...
def noise(waveforms,ch,sampling_freq,window=True,removeMean=True):
    if len(waveforms):
        if window is True:
            window=np.hanning(len(waveforms[0]))
        else:
            window=None
        spect = fft(wf=waveforms[0], fs=sampling_freq, window=None, removeMean=removeMean)
        freq = spect["f"]
        psd = spect["mod"]**2
        for wf in waveforms[1:]:
                psd += (fft(wf=wf, fs=sampling_freq, window=None, removeMean=True)["mod"])**2
